...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.progressbar_1 = customtkinter.CTkProgressBar(
            self.slider_progressbar_frame, width=450
        )
        self.progressbar_1.grid(
            row=1, column=1, padx=(90, 0), pady=(10, 10), sticky="nsew"
        )

        self.detect_label = customtkinter.CTkLabel(
            self,
            text="랜섬웨어 탐지를 위하여 스타트 버튼을 클릭하세요.",
            font=customtkinter.CTkFont(size=14),
        )  # , weight="bold"))
        self.detect_label.grid(row=2, column=1, padx=(20, 20), pady=(0, 0))

        # create textbox
        self.textbox = customtkinter.CTkTextbox(self, width=660, height=360)
        ...
        self.start_button = customtkinter.CTkButton(
            ...
            text="Ransomware Detection Start",
            font=customtkinter.CTkFont(size=18, weight="bold"),
            command=self.start_button_event,
        )  # , command=button_event)
        self.start_button.grid(
            row=4, column=1, padx=(20, 0), pady=(10, 10), sticky="nsew"
        )
        ...
        self.end_button = customtkinter.CTkButton(
            ...
            text="End Program",
            font=customtkinter.CTkFont(size=18, weight="bold"),
            command=self.end_button_event,
        )  # , command=button_event)
        self.end_button.grid(
            row=5, column=1, padx=(20, 0), pady=(10, 10), sticky="nsew"
        )

        self.progressbar_1.configure(
            mode="indeterminnate",
            indeterminate_speed=0.4,
            progress_color="Brown",
            corner_radius=40,
            width=450,
            height=80,
        )
    ...
    def start_button_event(self):
        ...
        # 파일 사전 생성
        test_directory = "C:\\test"
        if not os.path.exists(test_directory):
            os.makedirs(test_directory)

        ini_directory = "C:\\ini"
        ini_files = ["dection_time.ini", "cnt.ini"]

        for ini_file in ini_files:
            ini_path = os.path.join(ini_directory, ini_file)
...
class Handler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        app.textbox_update_event(f"{event.event_type},{event.src_path}")
        # try:
        #     app.textbox_update_event(
        #         f"{event.event_type},{event.src_path},{event.dest_path}"
        #     )
